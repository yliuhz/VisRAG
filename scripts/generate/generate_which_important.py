...
def main():
    ...
    qids, gt_doc_indexes, prefer_doc_indexes = [], [], []
    history_datas = []
    correct = 0
    total_num = 0
    ...
    for cnt, row in query_df.iterrows():
        ...
        if (use_positive_sample):
            ...
            for key, value in sorted(run[qid].items(), key=lambda item: item[1], reverse=True):
                if (doc_cnt < topk):
                    docid.append(key) ## docid
                    doc_scores.append(value) ## score
                    doc_cnt += 1
                else:
                    break
            if (len(docid) < topk):
        ...
        for gt_docid_item in gt_docid:
            if gt_docid_item in docid:
                gt_doc_index.append(docid.index(gt_docid_item)+1) ## start from 1
        ...
        if (task_type == 'text'):
            ...
            if (dataset_name == 'ChartQA'):
                input = [{'role': 'user', 'content': f'For given question "{query}", which image provided to you is the most important?\nAnswer with a single number. Do not answer the given question.\nAnswer:'}]
            elif (dataset_name == 'ArxivQA'):
                prompt = ''
                options = row['options']
                options_prompt = 'Options:\n'
                ...
                input = [{'role': 'user', 'content': f'For given question "{query}" and options "{options_prompt}", which image provided to you is the most important?\nAnswer with a single number. Do not answer the given question.\nAnswer:'}]
            elif (dataset_name == 'PlotQA'):
                input = [{'role': 'user', 'content': f'For given question "{query}", which image provided to you is the most important?\nAnswer with a single number. Do not answer the given question.\nAnswer:'}]
            elif (dataset_name == 'MP-DocVQA'):
                input = [{'role': 'user', 'content': f'For given question "{query}", which image provided to you is the most important?\nAnswer with a single number. Do not answer the given question.\nAnswer:'}]
        ...
        if (dataset_name == 'ChartQA'):
            ...
            print('---------------')
            if (is_numeric_data(responds) and int(float(responds)) in gt_doc_index):
                correct += 1
                correct_sig = True
        elif (dataset_name == 'ArxivQA'):
        ...
        prefer_doc_indexes.append(responds)

                
    output_dir = "/ssddata/liuyue/github/VisRAG/data/checkpoints/generator/which_important" # Write your output path here
    ...
    df.to_csv(os.path.join(output_dir, f"{prefix}_imp.csv"), index=False)
