...
[[ -d logs/generate/which_important_by_attn/$MODEL ]] || mkdir -p logs/generate/which_important_by_attn/$MODEL
...
CUDA_VISIBLE_DEVICES=4,5,6,7 \
    python -u scripts/generate/generate_which_important_by_attn.py \
        --model_name $MODEL \
        --dataset_name $DATA \
        --rank 0 \
        --world_size 1 \
        ...
        --ocr_type '' \
        2>&1 | tee logs/generate/which_important_by_attn/$MODEL/eval_g_${DATA}_${ORACLE}_${TOPK}.log
done
done
done
# done
