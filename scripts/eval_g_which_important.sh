...
[[ -d logs/generate/which_important/$MODEL ]] || mkdir -p logs/generate/which_important/$MODEL
...
CUDA_VISIBLE_DEVICES=0,1,2,3 \
    python -u scripts/generate/generate_which_important.py \
        --model_name $MODEL \
        --dataset_name $DATA \
        --rank 0 \
        --world_size 1 \
        ...
        --ocr_type '' \
        2>&1 | tee logs/generate/which_important/$MODEL/eval_g_${DATA}_${ORACLE}_${TOPK}.log
done
done
done
# done
